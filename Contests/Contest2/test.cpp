...
int main() {
	//boi vi deu la luy thua cua 2, nen neu so a b a voi b lon hon a 
	//thi chac chan a * 2 se be hon hoac bang b, do do cu nham vao so be hon ma loai bo no
	int n;
	cin >> n;
	vector<string> a(n);
	for (auto& x : a) cin >> x;
	stack<string> st;      //tao 1 stack luu gia tri tu lon den be  
							//va giu lai cac gia tri tiem nang trong nay
	for (auto x : a) {
		while (!st.empty() && isLessThan(st.top(), x)) st.pop();         
		//neu gia tri x lon hon dinh cua stack thi loai bo (vi dinh be hon x)
	...
	while (st.size() > 1) st.pop();//vi no giu lai cac gia tri tiem nang nen gia tri o 
					//day moi la lon nhat, loai bo toi khi con 1 phan tu
	//vi du 16 8
